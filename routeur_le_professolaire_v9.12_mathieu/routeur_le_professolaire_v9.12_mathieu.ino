...
const char* ssid = "IoT";                                // nom de votre réseau wifi
...
#define topic_mf_override "mosquitto/mf/set"
#define topic_mf "mosquitto/mf"
#define topic_surplus "mosquitto/surplus"
#define topic_surplus_virtuel "mosquitto/surplus_virtuel"
#define topic_ip "mosquitto/ip"
#define topic_dimmer_override "mosquitto/dimmer/set"
...
Card tempCard(&dashboard, GENERIC_CARD, "température ballon", "°C");
    ...
    if (client.connect(probe_name, mqtt_user, mqtt_password, will_topic, will_qos, will_retain, will_msg)) 
      {
        ...
        client.subscribe(topic_mf_override);
        client.subscribe(topic_enable_set);
        client.subscribe(topic_restart);
        client.subscribe(topic_dimmer_override);
        client.publish(topic_ip, WiFi.localIP().toString().c_str(), mqttRetainFlag);
        break;
      } 
...
if ( Auto == 0 )
    { 
      ...
      valDim1 = Value1;
      dimmer1.setState(dimmerOverride == -1 ? OFF : ON);
      dimmer1.setPower(dimmerOverride == -1 ? Value1 : dimmerOverride);
      valDim2 = Value2;
      dimmer2.setState(OFF);
      dimmer2.setPower(dimmerOverride == -1 ? Value2 : dimmerOverride);           
    }

if (mfOverride || marcheForceeVol == 1 && temps.getHours() == HOn & temps.getMinutes() == MnOn & temps.getSeconds() == SecOn && EnergyJ < energyNecessaireJ)
    {
      marcheForcee ();
    }
...
if (mfOverride || marcheForceeTemperature == 1 && temperatureC < temperatureEau && temps.getHours() == HOn & temps.getMinutes() == MnOn & temps.getSeconds() == SecOn)
    {
      Auto = 0;
    }
...
valDim1 = valDim1 + pas_dimmer;
      ...
      {
        dimmer1.setState(dimmerOverride == -1 ? OFF : ON);
        dimmer1.setPower(dimmerOverride == -1 ? minDimmer : dimmerOverride);
        valDim1 = minDimmer ;
        delay(60);
      }
      ...
      {
        dimmer1.setState(ON);
        dimmer1.setPower(dimmerOverride == -1 ? maxDimmer : dimmerOverride);
        valDim1 = maxDimmer ;
        delay(60);
      }
      ...
      {
        dimmer1.setState(ON);
        dimmer1.setPower(dimmerOverride == -1 ? valDim1 : dimmerOverride);
        delay(60);
      }
...
// réglages Dimmer 2 ///
       ...
       if ( valDim1 >= maxDimmer )
       {
        ...
        {
          dimmer2.setState(dimmerOverride == -1 ? OFF : ON);
          dimmer2.setPower(dimmerOverride == -1 ? minDimmer : dimmerOverride);
          valDim2 = minDimmer ;
          delay(60);
        }
        ...
        {
          dimmer2.setState(ON);
          dimmer2.setPower(dimmerOverride == -1 ? valDim2 : dimmerOverride);
          delay(60);
         }
...
// Mosquitto //
if (currentTime - previousTime2 >= mqttPublishIntervall )
   {
   client.loop();
  ...
  client.publish(topic, (String("{\"consommation\": ") + String(ajustePuissance) +" ,\"surplus\": " + String(Power1) +" , \"energie\": " + String(Energy1) +" , \"energytoday\": " + String(EnergyJ) +" , \"temperature\": " + String(temperatureC) +" }").c_str(), mqttRetainFlag);
  ...
  client.publish(topic_surplus_virtuel, String(ajustePuissance - Power1).c_str(),mqttRetainFlag);
  client.publish(topic_mf, Auto ? "false" : "true",mqttRetainFlag);
   }
